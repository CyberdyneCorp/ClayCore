...
|---|---|---|
| Standard | `Op::Relief` | 🟡 approximated, not matched: relief moves each point along its own normal (Inflate), Standard a whole stamp along one averaged normal. A few percent of the amplitude on smooth forms, the whole amplitude on a ridge narrower than the stamp, which relief thickens. One exact stamp is `clay_layer_move_surface` with a smoothstep ease; a stroke of them costs a warp per item per dab |
| ClayBuildup | `Op::Relief` along a stroke | ✅ buildup accumulation scales each stamp's amplitude. Its frame is relief's (each point's own normal) and has not been measured against the mesh `Clay` brush, which clamps the deposit to a plane |
| Smooth (as a pair with the above) | `field::relax` | ✅ the blockout pair — see `examples/29_claybuildup_smooth.py` |
| Crease, DamStandard | `Op::Incise` (SDF), `MeshBrush::Crease` (mesh), and on voxels a stroked erode with a CONSTANT falloff | ✅ on all three, the SDF form with relief's frame: `Op::Incise` cuts each point in along its own normal where a mesh Crease cuts along one shared direction, so on a ridge narrower than the stamp it severs the ridge rather than denting it. The voxel form is a recipe rather than a verb, and that is a measured decision: a crease verb was built and produced a profile identical to the plain erode, because the pinch that sharpens a mesh crease FILLS a voxel one — a lattice holds a volume, not a sheet |
| Inflate | `Op::Relief`, `sculpt_inflate` | ✅ relief is Inflate's frame exactly |
