...
**Adaptive topology is measured on hardware as of v0.78.0.** Dynamic topology
had no device case at all until the `ClayCoreDeviceDyntopoTests` bundle: 69
gated cases and not one of them touched the one sculpting mode that CHANGES the
mesh under the brush. Four cases now do, taken on the reference iPad15,5 on
iPadOS 26.5.2, thermally nominal start to end:
...
**† A high-detail dab does not fit a frame.** At `detail_resolution` 8 one dab
costs 5.40 ms against a 4.17 ms frame share, and `check_device_bench.py` reports
it on every run. That is the honest shape of the feature rather than a defect to
hide: a host that offers an artist a detail slider is offering this cost, and
the fix is a host-side one — cap the resolution, or spread the remesh — not a
...
hundredfold growth in document size — exponents +0.05, +0.01 and −0.17. The
deformation-only dab is not: **+0.405, a 7.8× rise on a document a hundred times
larger**, for work whose footprint does not change. That is the regression
`tests/unit/test_dynamic_scale.cpp` gates on desktop, showing up on hardware in
the one case with no remesher in it, and it is why the pair is worth more than
either half.

**`dyntopo_chunk_copy`'s −0.17 is the chunker, not a speed-up.** Its growth
exponent is negative because one chunk is a smaller unit of work on a bigger
surface: the same dab dirties chunks of 708, 462 and 279 vertices as the patch
grows, because the partition goes from 6 chunks to 64 to 517. Read the row as
the cost of copying one chunk, which is what a host loops over, and not as
transport getting cheaper. Draining the whole dirty set instead would fold the
brush's reach back into a number meant to be about transport.
...
| the multiresolution stamp | **unmeasured on device, named on the record** — `clay_multires_sculptor_stamp` has desktop numbers (the multires table) and no device case, because the harness drives fields and grids. `clay_dynamic_sculptor_stamp` was in this row until v0.78.0 and is now measured by `ClayCoreDeviceDyntopoTests`; the fixture it needed — import a triangle patch, build the adaptive surface, aim the dab at a fixed point — is the same one the mesh-brush rows above are still waiting on, so it is a worked example rather than a special case |
