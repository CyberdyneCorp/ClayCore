...
**Adaptive topology is measured but not yet baselined.** Dynamic topology had
no device case at all until the `ClayCoreDeviceDyntopoTests` bundle: 69 gated
cases and not one of them touched it, so the one sculpting mode that CHANGES
the mesh under the brush was the one with no hardware number. Four cases now
exist — `dyntopo_stamp`, `dyntopo_stamp_fixed`, `dyntopo_stamp_fine`,
`dyntopo_chunk_copy` — and they carry no figures here yet, because a baseline
has to be recorded on the reference iPad before anything in this document can
