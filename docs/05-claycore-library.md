...
constructors refuse and ask for a region, `clay_sdf_smooth_begin` refuses the
layer (its working lattice is derived from the box), a raycast and a pick skip
the clip and the per-ray local tape, `advised_params` gives no advice and the
prefix cache declines (the layer walks in full) — and a host reading `clay_tape_info` must test for ±FLT_MAX before it
...
- **A mesh is not reproducible across host toolchains, and never was.** Marching reads a floating-point field, so its output depends on floating-point contraction — on the compiler, the architecture and the optimisation level that built this library. Measured on one machine, one compiler, one flag apart: the same unit sphere at voxel 0.02 meshes to **281,568** triangles by default and **281,544** with `-ffp-contract=off` (#581). The engine pins no FP flags for its own translation units and makes no cross-build promise; what it guarantees is per-process — a sculpt's `indices` and `quads` come back byte for byte, the GPU meshers match the CPU reference on topology invariants rather than on vertex bits, and pyclay matches the CPU bit-for-bit because the CUDA kernels are compiled `-fmad=false` for exactly this reason (`docs/RELEASE.md`). A host that pins a triangle count, a vertex position or a mesh digest ACROSS platforms is resting on something that moves; `docs/08-mesh-readback.md` says it where a caller reads. `tests/unit/test_mesh_sculpt_parity.cpp` keeps one golden hash table per toolchain and records the FP configuration each was baselined under, so a build with different flags is told it has different flags instead of reporting 72 apparent arithmetic failures.
...
- **Decimation**: quadric edge collapse via `meshoptimizer`, color-attribute aware, target ratio or error. Quads do not survive it: edge collapse is a triangle operation and breaks the pairing on its first collapse, so a decimated quad mesh comes back as triangles. **It can pinch a surface, and it says when it did** — `mesh::DecimateReport`, and since ABI 0.115.0 `clay_decimate_report` / `clay_mesh_decimate_report` on the C boundary, which is where a host actually meets it. `manifold` is the result, `input_manifold` says whose pinch it is (only meaningful when `manifold` is false, since the input is examined only once the result is found bad) and `attempts` says whether the retry ladder ran. This is NOT recoverable by validating the result afterwards: by then the input mesh is gone. Nor is it confined to aggressive ratios — a bare unit sphere at voxel 0.02 comes back pinched at 31 of the 76 ratios from 0.20 to 0.95, the default 0.5 among them, and which ones move with floating-point contraction (issue #575), so a host that cannot take a non-manifold mesh reads the report on every decimated export.
...
- **Adaptive-surface undo** (`clay_dynamic_delta`, pyclay `TopologyDelta`, ABI 0.118.0): a host-owned record of one adaptive gesture (`mesh::RecordedGesture`, a `TopologyDelta` plus the surface marks at both ends), captured by `clay_dynamic_sculptor_stamp_recorded` and replayed by `clay_dynamic_delta_revert` / `_apply` **through the sculptor**. The host owns the sequence: how deep the stack is, when a record is dropped, how an undo interleaves with its own document edits. The engine owns one thing, the proof that a record matches the surface before it writes. **Records replay last in, first out, and nothing else.** Every surface carries a `{lineage, epoch}` mark, and a replay onto any state other than the record's end state is `CLAY_ERROR_SNAPSHOT_MISMATCH` with nothing written. This is needed even for strokes that never touch, because a later stroke reuses the slots an earlier one freed. Replaying onto the target state is `CLAY_OK` and writes nothing. The sculptor's chunk index and dirty-chunk stream follow the replay incrementally (0.1 ms at 49k faces against 39 ms for `rebuild_index`), so no rebuild is needed; one clears the dirty set and renumbers chunks. A replay restores `to_mesh` and the stored normals byte for byte over live elements, but not `clay_dynamic_surface_serialize` bytes, since dead slots stay. **Budgeting**: `clay_dynamic_delta_stats.resident_bytes` is what a record holds, allocator-dependent, and the memory ledger does not count it. `encoded_bytes` is exact (`56 + 122V + 114H + 42E + 66F`). **Spilling**: `clay_dynamic_delta_serialize` writes a record out and `_deserialize` reads it back to replay onto the same surface handle. It is not crash recovery, because a reloaded surface is a new lineage. One more limit: a replay keeps only the sculptor it runs through in step, so a second sculptor over the same surface is left stale, just as it is by a stamp. See `docs/07-brushes-and-features.md` § 8b.
