...
#undef CLAY_PLACE_ARGS
    ...
    nb::class_<PyMeshSculptor>(
        ...
        .def(
            ...
            "'grab' CARRIES THE REGION IT CAPTURED: it gathers once, at the first\n"
            "stamp, and every stamp places each captured vertex at its captured\n"
            "position plus its weight times the motion since the stroke BEGAN, so\n"
            "the surface moves as far as the cursor did. On a curve that is the\n"
            "net displacement and not the path length. 'snakehook' re-anchors on\n"
            "every stamp and drags by the motion between them, so its region walks\n"
    ...
    nb::class_<PyDynamicSculptor>(
        ...
        .def(
            ...
            "multiplies them. 'grab' carries the region it captured at the first\n"
            "stamp and moves it by the motion since the stroke began, with the\n"
            "remesh maintaining that region rather than rebuilding it and its\n"
            "centre following the stamp; 'snakehook' anchors on the vertex it\n"
            "drags, re-found near the previous stamp when the remesher retired\n"
            "it.\n\n"
            ...
            "region and Snakehook centre, which a loop of `stamp(record=)` does\n"
            "not. It\n"
