...
| Bindings | Stable C ABI (`clay.h`: item builder for composed edits, complete primitive/op/blend enumerations, versioned descriptor structs, the whole voxel engine incl. brushes, sculpting verbs, mask fields incl. the mask brush and mask extrude, and the stroke engine, picking and evaluation — parity with `pyclay` is gated in CI), SwiftPM xcframework, `pyclay` (nanobind, numpy-native: authoring incl. strokes and extended ops, voxels, evaluation, all three meshers, picking, I/O), `clay` CLI |
