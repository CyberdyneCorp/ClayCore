...
| Scene | Layers, ordered edit lists, nested groups, shared-content instancing, editing placed nodes (retransform, swap primitive, recolour, re-blend, move, remove) and layers (visibility, transform, reorder, remove) through one command vocabulary, influence bounds, flat postfix tape with per-brick culling, invertible+serializable undo commands |
