...
#### Scenario: A coloured volume reports its samples' colours
- **GIVEN** a volume whose samples carry two colours
- **WHEN** the tape is evaluated inside each coloured region
- **THEN** the colour reported is the one interpolated from that region's samples, not the item's constant

#### Scenario: A volume with no colour section is unchanged
- **GIVEN** a volume authored before the colour section existed
- **WHEN** the tape is evaluated inside it
- **THEN** the distance is what it always was and the colour is the item's own
