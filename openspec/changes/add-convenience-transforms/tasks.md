...
- [ ] 1.1 `clay_layer_snap_to_ground`, `clay_layer_centre_bounds` and
      `clay_layer_zero_to_origin` in `bindings/c/clay.h` and
      `bindings/c/clay_c.cpp`. No change to `src/`: the bound
      (`layer_world_bounds`) and the command (`scene::SetLayerTransformCmd`)
      both already exist and neither moves.
- [ ] 1.2 One shared helper on the inside taking the layer and a world-space
      ...
      clears the squash.
- [ ] 1.3 `zero_to_origin` sets `xform.position` to zero and reads no bounds —
      it shares 1.2's write path but not its bound query, which is why it
      accepts an empty and a radial layer.
- [ ] 1.4 The refusal set from `design.md`, in this order so a locked layer
      ...
      needs no case here — assert it rather than write it.
- [ ] 1.5 Header documentation beside the three, carrying what they do NOT
...
- [ ] 2.1 **The defect the change exists for**: a layer carrying three
...
- [ ] 2.2 Each rule against a hand-computed box: the snap puts `bounds_min.y`
...
- [ ] 2.3 The same three on an SDF, a voxel and a mesh layer, each with a
...
- [ ] 2.4 One undo step each, and one invalidation: undo restores the previous
      placement exactly, redo restores the new one.
- [ ] 2.5 The refusals, one case per row of the table in `design.md`, each
...
- [ ] 2.6 A hidden layer is accepted (bounds read content, not visibility) and
      a hidden ITEM is excluded from the box, so hiding the lowest root moves
      where the next snap lands.
- [ ] 2.7 An instance is placed and not severed: snap one of two layers sharing
...
- [ ] 2.8 Idempotence to a tolerance: a second snap moves the layer by at most
      one ulp at the box's magnitude. Not a bit-equality assertion — see
      `design.md`.

## 3. Python and the parity gate

- [ ] 3.1 `Layer.snap_to_ground(ground_y)`, `Layer.centre_bounds()` and
...
- [ ] 3.2 `python3 tools/check_binding_parity.py --pyclay <build>/bindings/python
...
- [ ] 3.3 No Swift work: `bindings/swift` is a link target
...
- [ ] 4.1 **ABI 0.85.0 -> 0.86.0**, moved in the PR that adds the entry points,
      not at release time. All three lines must agree: `CMakeLists.txt`
...
- [ ] 4.2 `python3 tools/check_c_abi.py` (header hygiene and the ctypes FFI) and
      `python3 tools/check_test_shards.py`.
- [ ] 4.3 `npx -y @fission-ai/openspec@1.8.0 validate --all --strict`.
- [ ] 4.4 `python3 tools/release_check.py --skip-slow` before pushing.
- [ ] 4.5 `docs/05-*` gains the three; `docs/RELEASE.md` at release time.
