...
**First, the model is the engine.** Engine Relief equals `a - k*S(1 - clamp(b/W,
0, 1))`, with `S(t) = t*t*(3 - 2t)`, `b = |p - c| - R - W` the rounded region's
field and `W` the rounding, to within 1.4e-7 at 20,000 points per fixture, and
...
**The hypothesis holds: Relief is Inflate.** The same holds at `k = 0.05` with
the same region and rounding (inflate_ref max 0.002k away from the fin's corner)
and with region, rounding and amplitude all doubled to 0.3, reach 0.90 (max
0.017k away from the fin's corner, on the bowl; the corner stays at 0.093k).
...
The fraction does not shrink with the amplitude: at `k = 0.05` draw_ref sits
0.024k / 0.102k / 0.039k / 0.598k (mean; sphere, saddle, bowl, fin) from the
relief surface, slightly worse than at 0.15. And when the stamp is wide against
the form (region, rounding and `k` all 0.3, reach 0.90), the difference stops
...
- **Its first-order form**, `a - k*w*(grad a . N)`, needs the accumulation's
  ...
  matters: measured with the normalized gradient `n(p)` in place of `grad a`
  (the two agree where the accumulation is a distance), it lies 0.129k mean /
  0.733k p95 from draw_ref on the fin. Its slope carries
...
- Correct every place that maps ZBrush Standard onto `Op::Relief` as though it
  were faithful: `bindings/c/clay.h` (the `CLAY_OP_RELIEF` and
  `CLAY_OP_INCISE` comments), `include/clay/kernel/tape.h`,
  `include/clay/scene/types.h`, `docs/07` §9, `docs/09`'s inventory and its
  coverage row, `docs/sculpt_comparison.md`, and `examples/25_relief.py`'s
  docstring. Relief
...
- **`move_surface`'s default ease is linear, not smoothstep**, and with it the
  fin's top rises 0.1115. Only `ease = smoothstep` (index 1) is the mesh Draw's
  profile, and even then a single stamp's peak is 0.1225, not the mesh's 0.1491:
  a warp reads its weight at the displaced point, where the mesh reads it at the
  vertex's rest position. That is the same residual floor as above, and why the
  comparison against the mesh brush is made with the falloff matched on both
  sides rather than by peak height.
- **Incise inherits the frame, and on a thin ridge it severs rather than
  dents.** It is the same kernel branch with `-k` (`ctape_combine_dist`), so
  everything above holds for it by construction; `docs/07` maps Crease and
  DamStandard onto it. Measured on the same fin at the standard mapping (review
  probe, pyclay from this tree, scanning the fin's axis from above): one incise
  stamp removes the fin through the whole weighted region — the top drops from
  0.500 to 0.108, the half-thickness reads 0 from y = 0.2 up and 0.012 at
  y = 0.1 — where a mesh Crease (`kernel_crease`, not measured here) cuts along
  one shared `deposit_direction` plus a tangential pinch. The header and `docs/07` corrections
  cover `CLAY_OP_INCISE` too.
- **ClayBuildup is not measured here.** Its SDF mapping is Relief along a stroke
  with buildup accumulation; the mesh `Clay` clamps Draw's deposit to a plane, a
  third behaviour. The frame result says Relief is not that either, but no
  number in this proposal is about it, so the correction names it unmeasured
  rather than approximated by a stated amount.
- **The main spec already says the frame.** `sdf-kernels` states that relief
  "displaces the existing surface along its normal". The ADDED requirement adds
  only the naming — Inflate, not Standard — and the obligation to document the
  divergence.
