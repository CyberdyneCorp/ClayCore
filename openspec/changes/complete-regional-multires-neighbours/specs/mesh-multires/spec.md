...
level's runtime cache. A neighbouring FACE SHALL belong to it only when that
face touches a vertex the level stores, so the faces follow the region's
boundary rather than the surface outside it.

It SHALL NOT charge a per-dab cost. A neighbourhood beyond the level can only
move when the level's PARENT moves, so an evaluation that did not move the
parent — a detail write, an undo replaying one, a recomposition — SHALL not
refresh it. Measured, refreshing on every evaluation cost 18x the
re-evaluation it was attached to.
...
#### Scenario: The neighbourhood's faces follow the boundary
...
#### Scenario: A dab does not pay for it
- **WHEN** a level is re-evaluated after a change that did not move its parent
- **THEN** the neighbourhood's positions are not recomputed
