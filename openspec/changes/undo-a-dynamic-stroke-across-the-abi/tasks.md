...
- [ ] 2.1 Regression test first, in `tests/unit/test_topology_delta.cpp` or
...
- [ ] 2.2 Confirm the test fails on the unfixed tree. It should report nonzero
      differing normals, matching measurement 1.5
- [ ] 2.3 `remesh_local.cpp` relax loop: collect the incident faces up front,
      note those faces and their vertices, write positions, `refresh_normals`,
      then sync them
- [ ] 2.4 The test passes. Re-run `test_topology_delta` and `test_dynamic_*` to
      confirm the record's growth changed no other assertion

## 3. Engine: mark, recorded gesture, replay

- [ ] 3.1 `DynamicSurface::mark()` / `set_mark()`. The lineage comes from an
...
- [ ] 3.2 Audit every writer of the pools and confirm that each one bumps a
      revision. A writer that does not bump would slip past the guard
- [ ] 3.3 `RecordedGesture` holds `TopologyDelta` plus the before and after
...
- [ ] 3.4 A const view of `TopologyDelta`'s face and vertex entries
- [ ] 3.5 A `DynamicSculptor::stamp` overload taking `RecordedGesture*`. It
      refuses capture into a bound, non-empty record whose `after` mark differs
      from the surface
- [ ] 3.6 `DynamicSculptor::replay(const RecordedGesture&, Direction)`, split
...
- [ ] 3.7 Measure the cognitive complexity of `replay` and its parts; the target
      is 15 or less each

## 4. C ABI (0.117.0 -> 0.118.0)

- [ ] 4.1 Bump `CMakeLists.txt`, `CLAY_ABI_MINOR` and `pyproject.toml` together
- [ ] 4.2 `clay_dynamic_delta` with `_create`, `_destroy`, `_clear`,
      `_stats_get`, `_revert`, `_apply`, `_serialize` and `_deserialize`, plus
      `clay_dynamic_sculptor_stamp_recorded`
- [ ] 4.3 `clay_dynamic_delta_stats` behind `struct_size`, with its original
      layout named by its last field
- [ ] 4.4 Extract the descriptor reading shared by `clay_dynamic_sculptor_stamp`
...
- [ ] 4.6 Header text on what the calls do NOT promise:
...
- [ ] 4.7 `tools/check_c_abi.py` passes against a freshly built
      `libclay_shared.dylib`. Update the struct mirror in the checker

## 5. Tests (counts, not clocks)

- [ ] 5.1 C ABI, new `tests/unit/test_c_dynamic_delta.cpp`:
      ...
      - N=8 strokes undone in reverse, then redone in order, match the export
        at every step
- [ ] 5.2 The byte cost as a count. `encoded_bytes == 56 + 122V + 114H + 42E +
...
- [ ] 5.6 A serialize/deserialize round-trip reverts identically. Truncated
...
- [ ] 6.2 `tools/check_binding_parity.py`: map `TopologyDelta` to the
...
- [ ] 6.3 `bindings/python/tests/test_dyntopo.py`: replace the "UNDO/REDO IS
      DELIBERATELY ABSENT" paragraph with undo/redo parity and refusal tests
- [ ] 6.4 `tests/swift/smoke.swift`: capture, revert, apply and stats beside the
      existing dynamic-surface block

## 7. Docs and gates

- [ ] 7.1 `docs/05-claycore-library.md`: an adaptive-surface undo section
      covering host-owned records, last-in first-out replay, budgeting with
      `resident_bytes` and spilling with serialize
- [ ] 7.2 The follow-up for `session::History` holding a stale sculptor index
      is recorded in the PR body, not fixed here
- [ ] 7.3 Reproduce the Ubuntu GCC `-Werror` flags where possible, run
