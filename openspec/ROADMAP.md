...
| `add-noise-field` | What Blob needs, and what nothing else here has: a noise source. The decisions are which noise, whether it lives in the kernel dialect so all four backends agree bit-for-bit, and how a seed reaches the tape. Unlocks Blob and a displacement that is not a sine. |
